...
contract OptimismPortal is Initializable, ResourceMetering, ISemver {
        ...
        metered(_gasLimit)
    {
        ...
        /// @notice Accepts deposits of LST and data, and emits a TransactionDeposited event for use in
    ///         deriving deposit transactions. Note that if a deposit is made by a contract, its
    ///         address will be aliased when retrieved using `tx.origin` or `msg.sender`. Consider
    ///         using the CrossDomainMessenger contracts for a simpler developer experience.
    /// @param _to         Target address on L2.
    /// @param _gasLimit   Amount of L2 gas to purchase by burning gas on L1.
    /// @param _value      LST value to mint to the recipient as an exchange from token.
    /// @param _data       Data to trigger the recipient with.
    function depositLstTransaction(
        address _to,
        uint64 _gasLimit,
        uint256 _value,
        bytes memory _data
        ...
        metered(_gasLimit)
    {
        // reverts when caller is not the messenger.
        require(msg.sender == systemConfig.l1CrossDomainMessenger(), "OptimismPortal: only the messenger is callable");
